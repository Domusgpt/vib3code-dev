...
const math: RendererAdapter["math"] = {
  ...
  polytopeWarp(point: Vec4, params: WarpParams): Vec4 {
    const [x, y, z, w] = point;
    const factor = 1 + params.tension * 0.1;
...
const telemetry: RendererAdapter["telemetry"] = {
  batch(events: TelemetryEvent[]) {
    if (events.length) {
      console.debug("[vib3-plus] batching telemetry", events.length);
    }
  },
  flush() {
    console.debug("[vib3-plus] flush telemetry");
  }
};

export class Vib3PlusAdapter implements RendererAdapter {
  readonly math = math;
...
export function createVib3PlusAdapter(): RendererAdapter {
  return new Vib3PlusAdapter();
}
